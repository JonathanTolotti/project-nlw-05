...
document.querySelector("#start_chat").addEventListener("click", (event) => {
  ...
  const btn_chat = document.getElementById("btn_support");
  ...
  socket.on("client_list_all_messages", (messages) => {
    ...
    divSupport.scrollTop = divSupport.scrollHeight;
  });
  ...
  socket.on("admin_join_chat", (adminID) => {
    socket_admin_id = adminID;
  });

  socket.on("admin_left_chat", () => {
    let element = document.querySelector(".message_send");
    element.remove();
  });
